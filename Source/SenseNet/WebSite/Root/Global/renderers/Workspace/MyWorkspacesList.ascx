        ...
        foreach (var wsg in wsGroupLists)
        {
            ...
            <div class="sn-content ui-helper-clearfix" style="margin-bottom: 5px; background-color: #FFF; border: solid 1px #DDD; padding: 5px;">
               ...
               for (var j = 0; j < groupInfos.Count; j++)
               {
                   ...
                   %><%= j > 0 ? "; " : "" %>in <a href="<%= SenseNet.ContentRepository.Content.Create(groupInfo.Group).Path%>"><%= SenseNet.ContentRepository.Content.Create(groupInfo.Group).DisplayName%></a><%= viaCount > 0 ? "" : "; "%><% 
                    ...
                    if (viaCount > 0 && !viaGroups.Select(g => g.Id).Contains(user.Id))
                    {
                       %> via <%
                        for (var i = 0; i < viaCount; i++)
                        {
                           ...
                           %><a href="<%= SenseNet.ContentRepository.Content.Create(group).Path%>"><%= SenseNet.ContentRepository.Content.Create(group).DisplayName%></a><%= i == viaCount - 1 ? "; " : ", "%><%
                        }
                    }
               } %>
                         </div>
