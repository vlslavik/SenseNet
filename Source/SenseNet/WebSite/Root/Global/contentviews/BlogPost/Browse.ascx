        ...
        <% 
            ...
            {%>
                <div class="sn-blogpost-unpublished">                    
                    <span><%= SenseNetResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_UnpublishedPost")%></span>
                </div>
            <%}
            else
            {
                ...
                if (showAvatar)
                { %>
                <div style="float: left; margin-right: 10px;"><img style="margin-top: 5px" src='<%= UITools.GetAvatarUrl(this.Content["CreatedBy"] as SenseNet.ContentRepository.User, 48, 48) %>' class='sn-entry-avatar' alt='<%# Eval("CreatedBy") %>' title='' /></div>
                <%} %>  
                <div>
                    <div class="sn-blogpost-admin">
                        <% var editUrl = Actions.ActionUrl(this.Content, "Edit", true);
                           ...
                           {%>
                                <div class="sn-blogpost-admin-item"><img src="/Root/Global/images/icons/16/edit.png" alt="<%=GetGlobalResourceObject("Content", "Edit")%>" /><a href="<%= editUrl %>"><%= SenseNetResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_EditPost")%></a></div>
                            <%}
