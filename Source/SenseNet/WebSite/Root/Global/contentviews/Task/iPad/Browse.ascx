            ...
            <% 
                ...
                if (users != null && users.Count > 0)
                {
                    foreach (var user in users)
                    {
                        ...
                        if (tmpUser != null)
                        {
                            var avatarUrl = SenseNet.Portal.UI.UITools.GetAvatarUrl(tmpUser, 32, 32);
                            var fullName = String.IsNullOrEmpty(tmpUser.FullName) ? tmpUser.DisplayName : tmpUser.FullName;                                                      
                            %><a class="snm-avatar-user clickable-yellowbg" style="background-image: url(<%= avatarUrl %>);" href="<%= SenseNet.Portal.Helpers.Actions.ActionUrl(SenseNet.ContentRepository.Content.Create(tmpUser), "Profile", true) %>" ><%= fullName %></a><%
                        }
                    }
                }
                else
