                ...
                <div>
                    ...
                    <% var i = 0;
                       foreach (string tag in content["Tags"].ToString().Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries)) { 
                        ...
                        i++;
                    %>
                    <%= i > 1 ? ", " : " - " %><a class="sn-blogpost-tag" href="<%= Actions.ActionUrl(wsContent, "Search", false) + "&text=" + SenseNet.ContentRepository.Security.Sanitizer.Sanitize(tag) %>"><%= SenseNet.ContentRepository.Security.Sanitizer.Sanitize(tag)%></a>
                    <% } %>
                </div>
