...
<div class="sn-contentlist sn-bloglist">
    <%
        ...
        <div class="sn-blogpost">
            <p class="sn-blogpost-missing"><%= SenseNetResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_NoPosts")%></p>
        </div>
        <%}
        else
        {
            ...
            <% if (showAvatar)
               { %>
                <div style="float: left; margin-right: 10px;">
                    <img class="sn-blogpost-avatar" style="margin-top: 5px" src='<%= UITools.GetAvatarUrl(content["CreatedBy"] as SenseNet.ContentRepository.User, 48, 48) %>' alt='<%# Eval("CreatedBy") %>' />
                </div>
            <%} %>            
            <div>
                <div class="sn-blogpost-admin">
                    <% var editUrl = Actions.ActionUrl(content, "Edit", true);
                       ...
                       {%>
                          <div class="sn-blogpost-admin-item"><img src="/Root/Global/images/icons/16/edit.png" alt="<%=GetGlobalResourceObject("Content", "Edit")%>" /><a href="<%= editUrl %>"><%= SenseNet.ContentRepository.i18n.SenseNetResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_EditPost")%></a></div>
                       <%}
                       ...
                       {%>
                           <div class="sn-blogpost-admin-item"><img src="/Root/Global/images/icons/16/delete.png" alt="<%=GetGlobalResourceObject("Content", "Delete")%>" />
                                <a href="javascript:void(0);" onclick="<%= deleteUrl %>" class="sn-actionlink"><%= SenseNetResourceManager.Current.GetString("Portal", "SnBlog_BlogPostsPortlet_DeletePost")%></a>
                            </div>
                       <%}
                    %>                    
                </div>
                <div class="sn-blogpost-info">
                    ...
                    %>
                        <%= i > 1 ? ", " : " - " %>
                        <a class="sn-blogpost-tag" href="<%= Actions.ActionUrl(wsContent, "Search", false) + "&text=" + SenseNet.ContentRepository.Security.Sanitizer.Sanitize(tag) %>"><%= SenseNet.ContentRepository.Security.Sanitizer.Sanitize(tag)%></a><% } %>
                 </div>
            </div>
