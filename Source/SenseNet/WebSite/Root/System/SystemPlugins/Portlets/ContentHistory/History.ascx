...
<asp:ListView ID="HistoryListView" runat="server" EnableViewState="false" DataSourceID="SNDSVersions"  >
    ...
    <ItemTemplate>
        <tr class='sn-lg-row<%# Container.DisplayIndex % 2 %> ui-widget-content'>      		
          ...
          <td class="sn-lg-col-2"><%# ((SNCR.Content)Container.DataItem).ContentHandler.VersionModificationDate.ToString() %> (<%# ((SNCR.Content)Container.DataItem).ContentHandler.VersionModifiedBy["Name"].ToString()%>)</td>
