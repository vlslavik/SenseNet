       	    ...
       	    <div class="snm-col">
                ...
                <% foreach (var ws in wsListCritical)
                    ...
                    <article class="snm-tile snm-tile-wide3 bg-primary clr-text">
                        <a href="<%= Actions.BrowseUrl(SenseNet.ContentRepository.Content.Create(ws)) %>" class="snm-link-tile"><span class="snm-lowertext snm-fontsize3"><%= HttpUtility.HtmlEncode(ws.DisplayName) %></span></a>
                    </article>
                <% } %>
                ...
                <% foreach (var task in taskList)
                    { %>
                    <article class="snm-tile snm-tile-wide3 bg-primary clr-text anim-zoomin">
                        <a href="<%= Actions.BrowseUrl(SenseNet.ContentRepository.Content.Create(task)) %>" class="snm-link-tile"><span class="snm-lowertext snm-fontsize3"><%= HttpUtility.HtmlEncode(task.DisplayName) %></span></a>
                    </article>
                <% } %>
                ...
                <% foreach (var doc in docList)
                    { %>
                    <article class="snm-tile snm-tile-wide3 bg-primary clr-text anim-zoomin">
                        <a href="<%= Actions.BrowseUrl(SenseNet.ContentRepository.Content.Create(doc)) %>" class="snm-link-tile"><span class="snm-lowertext snm-fontsize3"><%= HttpUtility.HtmlEncode(doc.DisplayName) %></span></a>
                    </article>
                <% } %>
