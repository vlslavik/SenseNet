        ...
        public override void SetData(object data)
        {
            ...
            if (SenseNetResourceManager.ParseResourceKey(displayName, out className, out name))
            {
                ...
                if (rescontrol != null)
                {
                    ...
                    {
                        Datas = ResourceEditorController.GetResources(className, name).Select(p => new SenseNet.Portal.ResourceEditorController.ResourceData { Lang = p.Key, Value = p.Value }).ToList(),
                        Name = displayName
                    };
